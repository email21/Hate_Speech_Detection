...
from data import load_data, construct_tokenized_dataset, hate_dataset, load_tokenizer
from arguments import add_common_args, add_train_args

def parse_args():
    """K-fold 학습을 위한 인자를 관리합니다."""
    parser = argparse.ArgumentParser(description="K-fold Cross-Validation Training Script")
...
if __name__ == "__main__":
    ...
    all_f1_scores = []
    tokenizer = load_tokenizer(args) # 토크나이저는 한 번만 로드
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(full_train_df, labels)):
        ...
        wandb.init(project="Hate_Speech_Detection_KFold", name=run_name_fold, config=vars(args), reinit=True)
